...
import { localUrl } from './baseUrl';
 ...
 export async function postApi(param) {
  const res = await fetch(`${localUrl}/rest/test.branch/catBranches`,{
    method : 'POST',
    body : JSON.stringify({
      "page": 0,
       "cat" : param,
...
export default async function generateRssCats(param) {
 ...
 allPosts.siteMap ? allPosts.siteMap.branches.forEach((post) => {
    feed.addItem({
     title: post.name,
     id: `${post.id}`,
     link: post.url,
   ...
   allPosts.siteMap ?  allPosts.siteMap.cats.forEach((post) => {
    feed.addItem({
     title: post.name,
     id: `${post.id}`,
     link: post.url,
