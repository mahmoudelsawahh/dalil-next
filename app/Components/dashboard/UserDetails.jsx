...
const UserDetails = () => {
  ...
  const [CurrentCatId , setCurrentCatId] = useState(0)
	const [HasAccountData, setHasAccountData] = useState(0)
  ...
  const sendData = ()=>{
    ...
    if(HasAccountData > 0){
        ...
        setLoading(true)
          fetch(`${localUrl}/rest/test.branch/updateBranch`,{
            method : 'POST',
  ...
  fetch(`${localUrl}/rest/test.branch/prepare`,{
    ...
    return data.hasBranch == 1 ? 
     (
      ...
      setCatId(data.subCatId),
      // setCatId(9),
      // setCurrentCatId(60),
      setCurrentCatId(data.catId),
      ...
      setHasAccountData(data.branchId),
      setSelectedLocation({
        lat: data.lat,
         lng: data.lang
     })
    ...
    <div style={{position : 'relative'}}>  
        ...
        <form onSubmit={sendData}>
              ...
              <option  value={0}  selected={CatId == 0}>التصنيف الاساسي</option>
									{Cat.map((item)=>{
										return (
												<option ref={ref} value={item.id} key={item.id} selected={item.id == CatId}>{item.name}</option>
												)
									})}
								</select>
						</div>
              ...
              <option  value={0}  selected={CurrentCatId == 0 }>التصنيف الفرعي</option>
									{SubCat.length > 0 ? 
										SubCat[0].catList.map((item)=>{
										return (
												<option value={item.id}  key={item.id} selected={item.id == CurrentCatId}>{item.name}</option>
												)
									}):
									null
									}
				...
				<div className="container-xxl">
					<div className="row align-items-center">
						<div className="col-md-12 col-12">
							<h1 className="breadcrumb-title"> عنوان النشاط </h1>
						</div>
					</div>
				</div>
     		 </div>
			  ...
			  <div className="breadcrumb-bar my-5">
				<div className="container-xxl">
					<div className="row align-items-center">
						<div className="col-md-12 col-12">
							<h1 className="breadcrumb-title"> صور النشاط </h1>
						</div>
					</div>
				</div>
     		 </div>
                ...
                >
                  ...
                  { ListImages.length > 0 || ListImageFromUser.length?
                     <div style={{display : 'flex' , flexWrap : 'wrap' , gap : 20}}>
                      ...
                      return (
                        <div key={id} className="image-item" style={{position : 'relative' , width : '100px', height : '100px'}}>
                          <Image
                          ...
                          <div className="image-item__btn-wrapper">
                            <div className="btn btn-danger" onClick={()=> handelRemove(item)} style={{position : 'absolute'}}>
                              <AiFillDelete/>
                            </div>
                          </div>
                        </div>
                      ...
                      {imageList.map((item, index) => (
                        <div key={index} className="image-item" style={{position : 'relative' , width : '100px', height : '100px'}}>
                          <Image
                            src={item["data_url"]}
                            alt="حذف"
                            style={{
